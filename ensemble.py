...
def machine_learning_model(userid,solo, business, short, listings, city, features):
    purpose = []
    if solo:
        purpose.append(1)
        purpose.append(0)
    ...
    city = listings[listings['city'] == city]
    city = city[features]
    model = joblib.load('gbr')
    final = []
    for i,r in city:
        test = [purpose.extend(city.iloc[i])]
        ans = model.precict(test)
...
gbrfeatures = pd.read_csv('./gbrdata.csv').drop(['reviewer_id','reviewer_name','listings', 'Unnamed: 0', 'id']).columns
...
def ensemble(userid, solotrip, business,shortstay,city):
    ...
    ml_filter = machine_learning_model(int(userid), solotrip, business, shortstay, listings, city, gbrfeatures)
